...
(function () {
    ...
    return [
      {
        ...
        purpose: 'Sitzungsverwaltung für GitHub-gehostete Seiten',
        type: 'HttpOnly',
      ...
      },
      {
        name: '_octo',
        service: 'GitHub Pages',
        purpose: 'GitHub-Analyse- und Tracking-Token',
        type: 'HttpOnly',
        category: 'Infrastruktur',
        active: true
      },
      {
        name: 'logged_in',
        service: 'GitHub',
        purpose: 'Zeigt den GitHub-Anmeldestatus an',
        type: 'HttpOnly',
        category: 'Infrastruktur',
        active: true
      },
      {
        ...
        purpose: 'Bot-Verwaltung und Challenge-Prüfung',
        type: 'Drittanbieter',
        category: 'Funktional',
      ...
      },
      {
        ...
        service: 'hCaptcha',
        purpose: 'Barrierefreiheits-Einstellungen für CAPTCHA-Challenges',
        type: 'Drittanbieter',
        category: 'Funktional',
        active: consent.functional
      },
      {
        name: 'hmt_id',
        service: 'hCaptcha',
        purpose: 'Geräte-Fingerabdruck zur Bot-Erkennung',
        type: 'Drittanbieter',
        category: 'Funktional',
        active: consent.functional
      },
      {
        name: 'firebase_auth',
        service: 'Firebase',
        purpose: 'Authentifizierungsstatus und Sitzungs-Token',
        type: 'Drittanbieter / IndexedDB',
        category: 'Funktional',
        active: consent.functional
      }
    ];
  ...
  function getKnownStorageKeys() {
    return [
      { name: 'theme', purpose: 'Einstellung für helles/dunkles Theme' },
      { name: 'viewMode', purpose: 'Bild-Ansichtsmodus des Blogs (z. B. Karussell)' },
      { name: 'headerSticky', purpose: 'Ob die Kopfzeile angeheftet bleibt' },
      { name: 'cookie_consent', purpose: 'Deine Cookie-Einwilligung (365 Tage gespeichert)' },
      { name: 'voted_', prefix: true, purpose: 'Abstimmungs-Dedup je Beitrag' },
      { name: 'ef:state:', prefix: true, purpose: 'Eingaben der Elektronik-Grundlagen-Widgets' }
    ];
  }
  ...
  function buildLocalStorageSectionHtml(items) {
    ...
    html += '<strong>Hinweis:</strong> <code>localStorage</code> bleibt auf deinem Gerät und wird niemals irgendwohin gesendet. ';
    html += 'Die Einträge, die diese Seite selbst setzt, sind <code>theme</code>, <code>viewMode</code>, <code>headerSticky</code>, ';
    html += '<code>cookie_consent</code>, <code>voted_*</code> (Abstimmungs-Dedup) und <code>ef:state:*</code> (Elektronik-Widgets). ';
    html += '<a href="/privacy/">Zur Datenschutzerklärung</a>';
    html += '</div>';
  ...
  function buildSection(id, emoji, title, count, isCollapsed, contentHtml) {
    ...
    var countLabel = (typeof count === 'string') ? count : (count === 0 ? 'leer' : count);
    var html = '';
  ...
  function buildCookieCardHtml(cookies) {
    if (!cookies.length) return '<p class="storage-section__empty">Für diese Domain wurden keine Cookies gefunden.</p>';
    ...
    cookies.forEach(function (c) {
      ...
      var sameSite = c.sameSite || 'k. A.';
      var expires = c.expires || 'Sitzung';
      ...
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Wert</span><span class="cookie-card__value">' + escapeHtml(truncate(c.value, 60)) + '</span></div>';
      ...
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Pfad</span><span class="cookie-card__value">' + escapeHtml(path) + '</span></div>';
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Sicher</span>' + flagHtml(secure) + '</div>';
      ...
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Läuft ab</span><span class="cookie-card__value">' + escapeHtml(truncate(expires, 30)) + '</span></div>';
      html += '  </div>';
      html += '</div>';
    });
    return html;
  ...
  function buildStorageCards(items) {
    if (items === null) return '<p class="storage-section__empty">Zugriff verweigert (privater Modus oder Speicher blockiert).</p>';
    if (!items.length) return '<p class="storage-section__empty">Keine Einträge gefunden.</p>';
    var html = '';
    items.forEach(function (item) {
      ...
      html += '  <div class="cookie-card__grid">';
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Wert</span><span class="cookie-card__value">' + escapeHtml(truncate(item.value, 120)) + '</span></div>';
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Typ</span><span class="cookie-card__value">' + type + '</span></div>';
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Größe</span><span class="cookie-card__value">' + size + '</span></div>';
      if (purpose) {
        html += '    <div class="cookie-card__field"><span class="cookie-card__label">Zweck</span><span class="cookie-card__value">' + escapeHtml(purpose) + '</span></div>';
      }
      html += '  </div>';
      html += '</div>';
    });
  ...
  function buildCacheCards(cacheData) {
    if (cacheData === null) return '<p class="storage-section__empty">Cache-Storage-API nicht verfügbar.</p>';
    if (!cacheData.length) return '<p class="storage-section__empty">Keine Caches gefunden.</p>';
    var html = '';
    cacheData.forEach(function (cache) {
      var urlCount = cache.urls.length;
      var maxUrls = 20;
      html += '<div class="cookie-card">';
      html += '  <div class="cookie-card__name">' + escapeHtml(truncate(cache.cacheName, 60)) + ' <span class="cookie-card__value" style="font-weight:400">(' + urlCount + ' Einträge)</span></div>';
      ...
      if (urlCount > maxUrls) {
        html += '    <li class="cache-url-list__more">und ' + (urlCount - maxUrls) + ' weitere…</li>';
      }
      html += '  </ul>';
      html += '</div>';
    });
  ...
  function buildKnownCookieCards(knownCookies) {
    if (!knownCookies.length) return '<p class="storage-section__empty">Keine bekannten Dienst-Cookies dokumentiert.</p>';
    var html = '';
    knownCookies.forEach(function (c) {
      var activeClass = c.active ? 'cookie-card__status--active' : 'cookie-card__status--inactive';
      var activeLabel = c.active ? 'Aktiv' : 'Inaktiv';
      ...
      html += '  </div>';
      html += '  <div class="cookie-card__grid">';
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Zweck</span><span class="cookie-card__value">' + escapeHtml(c.purpose) + '</span></div>';
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Typ</span><span class="cookie-card__value">' + escapeHtml(c.type) + '</span></div>';
      html += '    <div class="cookie-card__field"><span class="cookie-card__label">Kategorie</span><span class="cookie-card__value">' + escapeHtml(c.category) + '</span></div>';
  ...
  function buildCookieSectionHtml(cookies, knownCookies) {
    ...
    // Info note
    html += '<div class="storage-section__info">';
    html += '<strong>Hinweis:</strong> JavaScript kann keine HttpOnly- oder Drittanbieter-Cookies auslesen. ';
    html += 'Die &bdquo;Bekannten Dienst-Cookies&ldquo; unten sind von den auf dieser Seite eingebundenen Diensten dokumentiert. ';
    html += '<a href="/privacy/">Zur Datenschutzerklärung</a>';
    html += '</div>';

    // Detected cookies subsection
    html += '<h4 class="storage-subsection__title">Erkannte Cookies</h4>';
    html += '<div class="storage-subsection__detected">';
    if (!cookies.length) {
      html += '<p class="storage-section__empty">Diese Seite nutzt localStorage für ihre eigenen Daten. ';
      html += 'Über <code>document.cookie</code> wurden keine Erstanbieter-Cookies erkannt.</p>';
    } else {
    ...
    html += '<h4 class="storage-subsection__title">Bekannte Dienst-Cookies</h4>';
  ...
  function buildModalContent() {
    ...
    var cookieCountLabel = cookies.length + ' erkannt \u00b7 ' + knownCookies.length + ' bekannt';
    ...
    if (hasCacheApi) {
      html += buildSection('cache', '📦', 'Cache-Speicher', '…', true, '<p class="storage-section__loading">Cache-Daten werden geladen…</p>');
    }

    if (totalSync === 0 && !hasCacheApi && knownCookies.length === 0) {
      cookieBody.innerHTML = '<p class="cookie-empty">Für diese Domain wurden keine Browser-Speicherdaten gefunden.</p>';
      return;
    }
    ...
    parseCookiesAsync(function (richCookies) {
      ...
      if (badge) badge.textContent = richCookies.length + ' erkannt \u00b7 ' + getKnownCookies().length + ' bekannt';
