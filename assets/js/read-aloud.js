(function () {
  ...
  // --- State ---
  var sentences = [];
  var readLang = document.documentElement.lang || 'de';
  var currentIndex = -1;
  var isPaused = false;
  ...
  function extractSentences() {
    ...
    if (!body) return [];
    readLang = body.getAttribute('lang') || document.documentElement.lang || 'de';
  ...
  function showPlayIcon() {
    ...
    playBtn.setAttribute('aria-label', 'Vorlesen');
    playBtn.setAttribute('title', 'Vorlesen');
    // Sync sticky bar
    if (stickyIconPlay) stickyIconPlay.style.display = '';
    if (stickyIconPause) stickyIconPause.style.display = 'none';
    if (stickyLabel) stickyLabel.textContent = 'Pausiert';
  }
  ...
  function showPauseIcon() {
    ...
    playBtn.setAttribute('title', 'Vorlesen pausieren');
    // Sync sticky bar
    if (stickyIconPlay) stickyIconPlay.style.display = 'none';
    if (stickyIconPause) stickyIconPause.style.display = '';
    if (stickyLabel) stickyLabel.textContent = 'Wird vorgelesen…';
  }
  ...
  function populateVoices() {
    var voices = synth.getVoices();
    // Prefer voices matching the page language (readLang, e.g. "de"), then all.
    var base = (readLang || 'de').slice(0, 2).toLowerCase();
    var preferred = voices.filter(function (v) {
      return v.lang && v.lang.slice(0, 2).toLowerCase() === base;
    });
    var list = preferred.length > 0 ? preferred : voices;
